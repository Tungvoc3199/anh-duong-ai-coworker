...
CHECKPOINT = Path("/mnt/f/AIOS/anh-duong-checkpoints/AD-RECOVERY-APPROVAL-REPLAY-1")
...
def point(target):
    staging = ROOT / "current.recovery-replay-stage"
...
def main():
    ...
    if RELEASE.parent != ROOT or RELEASE.name in {"current", "bca9409"}:
        raise SystemExit("Run the script from the prepared immutable release.")
    if CURRENT.resolve().name != "bca9409":
