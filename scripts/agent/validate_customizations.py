...
# "Command 'python' not found". Require the stable release interpreter path.
RELEASE_CURRENT = Path("/home/thadc/AIOS/releases/anh-duong-core/current")
VENV_PYTHON = f"{RELEASE_CURRENT}/.venv/bin/python"
BARE_PYTHON = re.compile(r"(?<![\w./-])python3?\s+(?=-m\b|-c\b|[\w./-]+\.py\b|scripts/)")

def is_reserved_release_path(path: Path) -> bool:
    """Allow static validation before the immutable current release is created."""
    return path == RELEASE_CURRENT or RELEASE_CURRENT in path.parents
        ...
        for entry in entries:
            ...
            for launcher in SHELL_LAUNCHERS:
                if launcher in command:
                    errors.append(
                        f"{path}: {event} command must not use a shell launcher ({launcher!r})"
                    )
            if cwd:
                cwd_path = Path(cwd)
                if not cwd.startswith("/"):
                    errors.append(
                        f"{path}: {event} cwd must be absolute, got {cwd!r}"
                    )
                elif not cwd_path.is_dir() and not is_reserved_release_path(cwd_path):
                    errors.append(f"{path}: {event} cwd does not exist: {cwd}")
            parts = command.split()
            interpreter = parts[0]
            if not interpreter.startswith("/"):
                errors.append(
                    f"{path}: {event} interpreter must be an absolute path, got {interpreter!r}"
                )
            elif not Path(interpreter).is_file() and not is_reserved_release_path(
                Path(interpreter)
            ):
            ...
            if len(parts) > 1:
                ...
                if (
                    script.startswith("/")
                    and not Path(script).is_file()
                    and not is_reserved_release_path(Path(script))
                ):
                    errors.append(f"{path}: {event} script not found: {script}")
                elif not script.startswith("/"):
                    errors.append(
                        f"{path}: {event} script must be an absolute path, got {script!r}"
                    )
    return errors
...
def validate(path: Path) -> list[str]:
    ...
    for required in REQUIRED.get(suffix, {"description"}):
        if required not in values:
            errors.append(f"{path}: missing required frontmatter key {required}")
    if not values.get("description", "").strip('"\' '):
        errors.append(f"{path}: blank description")
    return errors
